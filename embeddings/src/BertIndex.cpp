...
// Future improvement: add a reset_search_ctrl()
//
...
BertIndex::~BertIndex() {
    ...
    try {
        ...
        if (cfg.debug) {
            LOG_ERROR_S() << "[BertIndex::~BertIndex] flush/remove_lockfile failed: " << exception.what();
        }
    }
}
...
size_t BertIndex::append(const std::string &sentence, int64_t sentence_id) {
    ...
    if (embedding_util::isHexFloat32Vector(sentence, embedder.n_embd)) {
    ...
    for (auto &chunk : chunks) {
        ...
        // encode & add
        if (insert_raw) {
          auto emb = embedding_util::hexToFloat32 (chunk.text);
