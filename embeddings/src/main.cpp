...
int main(int argc, char **argv) {
    ...
    bool debug = false;
    std::optional<hnswlib::Metric> selected_metric;
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (a == "--metric" && i+1 < argc) {
            string m = argv[++i];
            if (m == "l2") selected_metric = hnswlib::Metric::L2;
            else if (m == "ip") selected_metric = hnswlib::Metric::IP;
            else if (m == "cos") selected_metric = hnswlib::Metric::Cosine;
            else {
                cerr << "Unknown metric '" << m << "'. Supported: l2, ip, cos\n";
                return 1;
            }
        }
    }
    ...
    ConfigLoader loader;
    hnswlib::HnswConfig cfg = loader.load(debug);
    if (selected_metric) cfg.set_metric(*selected_metric);
    ...
    continue;
        ...
        if (line.rfind("showfull ", 0) == 0) {
            ...
            if (line == "merge") {
                ...
                cout << "Merge completed (if any shards were mergeable).\n";
                continue;
            }
            if (line == "flush") {
